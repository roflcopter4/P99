...
#define P99_FIFO_INITIALIZER(HEAD, TAIL)                                              \
        {                                                                             \
                .p00_head = ATOMIC_VAR_INIT(HEAD), .p00_tail = ATOMIC_VAR_INIT(TAIL), \
        }
...
#define P99_FIFO_APPEND(L, EL)                                                                              \
        do {                                                                                                \
                /* first evaluate the macro arguments such that there can't be name conflicts */            \
                ...
                for (;;) {                                                                                  \
                        if (p00_head) {                                                                     \
                                ...
                                if (atomic_compare_exchange_weak_explicit(                                  \
                                        p00_h, &p00_head, 0, memory_order_acq_rel,                          \
                                        memory_order_relaxed)) {                                            \
                                        /* make p00_el the last element */                                  \
                                        atomic_exchange_explicit(p00_t, p00_el, memory_order_acq_rel)       \
                                            ->p99_fifo = p00_el;                                            \
                                        /* unlock the fifo */                                               \
                                ...
                                if (!p00_tail && atomic_compare_exchange_weak_explicit(                     \
                                                     p00_t, &p00_tail, p00_el, memory_order_acq_rel,        \
                                                     memory_order_relaxed)) {                               \
        ...
        p99_extension({                                                                           \
                ...
                for (;;) {                                                                        \
                        if (p00_head) {                                                           \
                                ...
                                register const P99_MACRO_VAR(                                     \
                                    p00_tail, atomic_load_explicit(p00_t, memory_order_consume)); \
                                if (!p00_tail)                                                    \
                                        break;                                                    \
                                p00_head = atomic_load_explicit(p00_h, memory_order_relaxed);     \
                        }                                                                         \
...
#define P00_FIFO_TABULATE(TYPE, TAB, ID, L)                                       \
        ...
        for (TYPE *p00_e = P99_PASTE2(ID, _head); p00_e; p00_e = p00_e->p99_fifo) \
                ++P99_PASTE2(ID, _cnt);                                           \
        TYPE *TAB[P99_PASTE2(ID, _cnt)];                                          \
        for (TYPE **p00_t = &(TAB[0]), *p00_e = P99_PASTE2(ID, _head); p00_e;     \
             p00_e = p00_e->p99_fifo, ++p00_t)                                    \
        *p00_t = p00_e
