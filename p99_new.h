...
p99_inline unsigned char (*p00_memcpy(size_t p00_len,
...
p99_inline unsigned char (*p00_initialize(size_t p00_len,
                                          unsigned char (*p00_base)[p00_len],
                                          size_t p00_init))[]
{
        for (; (p00_init * 2u) <= p00_len; p00_init *= 2u) {
                p00_memcpy(p00_init, P99_ASUB(p00_base, unsigned char, p00_init, p00_init),
                           P99_ASUB(p00_base, unsigned char const, 0, p00_init));
        }
        if (p00_len > p00_init) {
                p00_len -= p00_init;
                p00_memcpy(p00_len, P99_ASUB(p00_base, unsigned char, p00_init, p00_len),
...
#  define P00_ABLESS(X, ...) \
        ((P99_TYPEOF(__VA_ARGS__) * restrict)(unsigned char(*)[sizeof(__VA_ARGS__)]){X})
#else
#  define P00_ABLESS(X, ...) \
        ((void *restrict)(unsigned char(*)[sizeof(__VA_ARGS__)]){X})
#endif

#define P00_APLAIN(X, N) ((unsigned char(*)[N])(X))
#define P99_APLAIN(...)                      \
        (P99_IF_LT_2(P99_NARG(__VA_ARGS__))( \
            P00_APLAIN(__VA_ARGS__, sizeof(*__VA_ARGS__)))(P00_APLAIN(__VA_ARGS__)))

p99_inline void *
p00_memset(void *p00_tar, void const *p00_src, size_t p00_size, size_t p00_nb)
{
        p00_initialize(p00_nb * p00_size,
                       P00_APLAIN(memcpy(p00_tar, p00_src, p00_size), p00_size), p00_size);
        return p00_tar;
}
...
#define P99_ALLOC(...)                      \
        P99_IF_GT(P99_NARG(__VA_ARGS__), 1) \
        (P00_ALLOC(__VA_ARGS__))(P00_VMALLOC(__VA_ARGS__))
...
p99_inline void *
p00_calloc(void const *p00_src, size_t p00_size, size_t p00_nb)
{
        return p00_memset(malloc(p00_size * p00_nb), p00_src, p00_size, p00_nb);
}

#define P00_CALLOC0(T, N) \
        p00_calloc((void const *)&P99_LVAL(const T), sizeof(T), N)
...
P00_DOCUMENT_TYPE_ARGUMENT(P99_CALLOC, 0)
#  define P99_CALLOC(...) \
        P00_CALLOC(P99_CALL_DEFARG_LIST(P00_CALLOC, 2, __VA_ARGS__))
#endif
...
#define P99_NEW(...)                        \
        ...
        (P00_NEW(__VA_ARGS__))(P00_NEW_ARGS(__VA_ARGS__))
...
#  define P99_DEFINE_DELETE(T) \
        P99_INSTANTIATE(void, P99_PASTE2(T, _delete), T const *)
#else
P00_DOCUMENT_TYPE_ARGUMENT(P99_DECLARE_DELETE, 0)
...
#define P00_SIZEOF2(T, ...) \
        sizeof(P99_TOKJOIN(., P99_LVAL(const T), __VA_ARGS__))
...
#  define P99_SIZEOF(T, F, ...)               \
        P99_IF_EQ(P99_NARG(__VA_ARGS__), 1) \
        (sizeof(__VA_ARGS__))(P00_SIZEOF2(__VA_ARGS__))
#else
#  define P99_SIZEOF(...)                     \
        P99_IF_EQ(P99_NARG(__VA_ARGS__), 1) \
        (sizeof(__VA_ARGS__))(P00_SIZEOF2(__VA_ARGS__))
