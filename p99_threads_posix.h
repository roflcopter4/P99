...
# error "p99_threads.h not included somehow"
#endif
...
thrd_current(void)
{
        ...
        if (P99_UNLIKELY(!p00_loc)) {
                size_t p00_nb =
                    atomic_fetch_add_explicit(&p00_foreign_nb, 1, memory_order_acq_rel);
                if (!p00_nb)
                        atexit(p00_foreign_cleanup);
                if ((p00_nb ^ (p00_nb - 1)) == (p00_nb + (p00_nb - 1))) {
                        p00_foreign_tab =
                            realloc(p00_foreign_tab, sizeof(p00_thrd * [2 * (p00_nb + 1)]));
                }
...
P99_CONST_FUNCTION
p99_inline int
thrd_equal(thrd_t p00_thr0, thrd_t p00_thr1)
{
...
P99_WARN_UNUSED_RESULT
p99_inline int
cnd_timedwait(cnd_t *restrict p00_cond,
              mtx_t *restrict p00_mtx,
...
P99_WARN_UNUSED_RESULT
p99_inline int
cnd_wait(cnd_t *p00_cond, mtx_t *p00_mtx)
{
        return pthread_cond_wait(&P99_ENCP(p00_cond), &P99_ENCP(p00_mtx)) ? thrd_error
                                                                          : thrd_success;
}
...
P99_WARN_UNUSED_RESULT
p99_inline int
mtx_init(mtx_t *p00_mtx, int p00_type)
{
        if (p00_type & mtx_extras) {
                ...
                case mtx_normal:
                        p00_ret = PTHREAD_MUTEX_NORMAL;
                        break;
                case mtx_errorcheck:
                        p00_ret = PTHREAD_MUTEX_ERRORCHECK;
                        break;
                case mtx_recursive:
                        p00_ret = PTHREAD_MUTEX_RECURSIVE;
                        break;
                default:
                        return thrd_error;
                }
...
P99_WARN_UNUSED_RESULT
p99_inline int
mtx_timedlock(mtx_t *restrict p00_mtx, const struct timespec *restrict p00_ts)
{
...
P99_WARN_UNUSED_RESULT
p99_inline int
thrd_create(thrd_t *p00_thr, thrd_start_t p00_func, void *p00_arg)
{
...
thrd_detach(thrd_t p00_thr)
{
        ...
        if (atomic_flag_test_and_set_explicit(&P99_ENC(p00_thr)->p00_detached,
...
thrd_exit(int p00_res)
{
        ...
        fprintf(stderr,
                ...
                "P99: There is no consistent way we can transmit that code to the "
                "environment\n",
                p00_res);
        pthread_exit(0);
}
...
P99_WARN_UNUSED_RESULT
p99_inline int
thrd_join(thrd_t p00_thr, int *p00_res)
{
        void *p00_res0;
        if (P99_UNLIKELY(pthread_join(P99_ENC(p00_thr)->p00_id, &p00_res0)))
                return thrd_error;
        if (p00_res)
                *p00_res = P99_ENC(p00_thr)->p00_ret;
        free(P99_ENC(p00_thr));
        return thrd_success;
}
...
P99_WARN_UNUSED_RESULT
p99_inline int
thrd_sleep(const struct timespec *p00_duration, struct timespec *p00_remaining)
{
        errno       = 0;
...
P99_SETJMP_INLINE(p00_thrd_create)
void *
p00_thrd_create(void *p00_context)
{
        ...
        P00_THRD_LOCAL      = p00_cntxt;
        {
                ...
                if (atomic_flag_test_and_set_explicit(&p00_cntxt->p00_detached,
                                                      memory_order_acq_rel)) {
                        free(p00_cntxt);
                }
        }
