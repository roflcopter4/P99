...
#  define P00_BLK_GEN(BEFORE, COND, ...) \
        for (P00_ROBUST(BEFORE); (COND) && P00; (__VA_ARGS__), P00 = 0)
 ...
 **/
#ifdef __GNUC__
#  define P99_MARK(X) p99_extension({ __asm__ volatile("# " P99_STRINGIFY(__LINE__ : X)); })
#else
...
#define P99_UNWIND_PROTECT                                                                                \
        ...
        P00_BLK_BEFAFT(auto p00_jmp_buf p00_unwind_return = P00_ROBUST(P00_JMP_BUF_INITIALIZER),          \
                            /* returning can only be on because it was set by P99_UNWIND_RETURN and
                             * we fell of the edge after all P99_PROTECT clauses */ \
        ...
        switch (!setjmp(p00_unwind_top[0].p00_buf))                                                       \
                ...
                default: p00_code = p00_unwind_top[0].p00_code; p00_unw = !!p00_code;                     \
                } else                                                                                    \
                case 1:

p99_inline void
p00_unwind(void *p00_top, unsigned p00_level, int p00_cond)
{
...
#define P99_UNWIND_RETURN                                                           \
        ...
        case 1:                                                                     \
                ...
                case 0: P99_ALLOW(RETURN) return
...
#define P99_PROTECT                                                          \
        P99_DECLARE_INHIBIT(RETURN);                                         \
        if (0) {                                                             \
        case 0: p00_code = p00_unwind_top[0].p00_code; p00_unw = !!p00_code; \
        }                                                                    \
        P00_UNCASE
...
#define P99_INVARIANT(EXPR)                                                   \
        P99_PROTECTED_BLOCK(assert((EXPR) && "failed at beginning of block"), \
                            assert((EXPR) && "failed at end of block"))
