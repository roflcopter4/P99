...
#ifndef P99_ATOMIC_H
# error "never include this file directly, use p99_atomic.h, instead"
#endif

#include "p99_atomic.h" // For clangd


p99_inline uint8_t
p00_atomic_exchange_1(uint8_t volatile *p00_objp, uint8_t p00_ret)
{
      __asm__ volatile ("lock xchgb %1, %b0"
                        : "=r"(p00_ret)
...
p99_inline uint16_t
p00_atomic_exchange_2(uint16_t volatile *p00_objp, uint16_t p00_ret)
{
      __asm__ volatile ("lock xchgw %1, %w0"
                        : "=r"(p00_ret)
                        : "m"(*p00_objp), "0"(p00_ret)
                        : "memory");
      return p00_ret;
...
p99_inline uint32_t
p00_atomic_exchange_4(uint32_t volatile *p00_objp, uint32_t p00_ret)
{
      __asm__ volatile ("lock xchgl %1, %k0"
                        : "=r"(p00_ret)
                        : "m"(*p00_objp), "0"(p00_ret)
                        : "memory");
      return p00_ret;
...
p99_inline uint64_t
p00_atomic_exchange_8(uint64_t volatile *p00_objp, uint64_t p00_ret)
{
...
p99_inline void
p00_sync_lock_release_internal(uint32_t volatile *p00_objp)
{
...
p99_inline void
p00_mfence_internal(memory_order p00_ord)
{
...
#define p00_mfence(...)         \
      ...
      (p00_mfence_internal(memory_order_seq_cst))(p00_mfence_internal(__VA_ARGS__))
